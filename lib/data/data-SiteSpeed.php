...
if($gapi_speed_primary_dimension <> "" && $gapi_speed_secondary_dimension <> ""){
  $array = array(
	'dimensions'  => 'ga:'.$gapi_speed_primary_dimension.',ga:'.$gapi_speed_secondary_dimension,
        'sort'        => '-ga:pageviews',
        'max-results' => $gapi_speed_max_results_list
  );
} elseif($gapi_speed_primary_dimension <> ""){
  $array = array(
        'dimensions'  => 'ga:'.$gapi_speed_primary_dimension,
        'sort'        => '-ga:pageviews',
        'max-results' => $gapi_speed_max_results_list
  );    
} else {
...
$results = $analytics->data_ga->get(
  ...
  $fromDate,
  $toDate, 'ga:avgPageLoadTime,ga:pageviews,ga:bounceRate,ga:exitRate,ga:pageValue,ga:goalCompletionsAll,ga:goalConversionRateAll', $array);	
...
if($gapi_speed_primary_dimension <> "" && $gapi_speed_secondary_dimension <> ""){
	...
	foreach( $rows as $row ) {
	  $data[] = array(
		'primaryDimension'   => $row[0],
		'secondayDimension' => $row[1],
		'avgPageLoadTime'  => $row[2],
		'pageviews' => $row[3],
		'bounceRate' => $row[4],
		'exitRate' => $row[5],
		'pageValue' => $row[6],
		'goalCompletionsAll' => $row[7],
	...
	foreach( $rows as $row ) {
	  $data[] = array(
		'primaryDimension' => $row[0],
		'avgPageLoadTime'  => $row[1],
		'pageviews' => $row[2],
		'bounceRate' => $row[3],
		'exitRate' => $row[4],
		'pageValue' => $row[5],
		'goalCompletionsAll' => $row[6],
